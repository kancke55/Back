...
router.post('/resend-confirmation', async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'C�digo de confirma��o reenviado com sucesso. Verifique seu email.' });
  } catch (error) {
    console.error('Erro ao reenviar confirma��o:', error.message);
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    if (data.action !== 'reset_password' || !data.email) {
      return res.status(400).json({ message: 'Token de recuperao inválido.' });
    }
    ...
    return res.status(400).json({ message: 'Token de recuperao inválido ou expirado.' });
  }
});

router.post('/test/send-confirmation-email', async (req, res) => {
  if (process.env.NODE_ENV === 'production') {
    return res.status(404).json({ message: 'Rota de teste indisponível em produção.' });
  }
  ...
  try {
    ...
    console.error('Erro ao enviar e-mail de verificação de teste:', error.message || error);
    return res.status(500).json({ message: 'Erro ao enviar e-mail de verificação de teste.' });
  }
});
